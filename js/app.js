...
function handleFormInput(event) {
  ...
  saveState();
  updateSaveStatus('تم حفظ البيانات بنجاح.');
...
function validateStep(step) {
  ...
  fields.forEach((field) => {
    ...
    if (fieldIsValid && field.name === 'website' && value && !/^https:\/\//i.test(value)) {
      showFieldError(field, 'يرجى إدخال رابط صحيح يبدأ بـ https://');
      fieldIsValid = false;
    }
...
function startOver() {
  if (!window.confirm('هل ترغب في إعادة تعيين النموذج وإزالة البيانات المحفوظة؟')) {
    return;
  }
